...
describe('Auth API Endpoints', () => {
    ...
    describe('POST /api/auth/register', () => {
        ...
        it('should successfully register a user and set a refresh token cookie', async () => {
            ...
            user_service_1.UserService.updateRefreshToken.mockResolvedValue([1]);
            const response = await (0, supertest_1.default)(app_1.default)
                .post('/api/auth/register')
    ...
    describe('POST /api/auth/login', () => {
        ...
        it('should fail login if password check fails', async () => {
            ...
            user_service_1.UserService.getUserByEmail.mockResolvedValue(mockUser);
            const response = await (0, supertest_1.default)(app_1.default)
                .post('/api/auth/login')
                .send({ email: 'jane@example.com', password: 'wrongPassword' });
            expect(response.status).toBe(401);
    ...
    describe('POST /api/auth/refresh', () => {
        ...
        it('should fail if token does not match the one stored in DB', async () => {
            const refreshToken = jsonwebtoken_1.default.sign({ id: 'mocked-uuid-12345', email: 'jane@example.com', tokenId: 'abc' }, jwtRefreshSecret, { expiresIn: '7d' });
    ...
    describe('POST /api/auth/logout', () => {
        it('should fail logout if not authenticated', async () => {
            const response = await (0, supertest_1.default)(app_1.default).post('/api/auth/logout').send();
            expect(response.status).toBe(401);
        });
        it('should logout successfully and clear the refresh token cookie', async () => {
            const accessToken = jsonwebtoken_1.default.sign({ id: 'mocked-uuid-12345', email: 'jane@example.com' }, jwtSecret, { expiresIn: '15m' });
            const mockUser = {
                id: 'mocked-uuid-12345',
                name: 'Jane Doe',
                email: 'jane@example.com',
            ...
            user_service_1.UserService.getUserById.mockResolvedValue(mockUser);
            user_service_1.UserService.updateRefreshToken.mockResolvedValue([1]);
            const response = await (0, supertest_1.default)(app_1.default)
            ...
            expect(user_service_1.UserService.updateRefreshToken).toHaveBeenCalledWith('mocked-uuid-12345', null);
